task_prompt_json = """
你是一名“多无人机任务规划与遥感分析专家”（UAV Swarm Mission Planner / Remote Sensing Specialist）。
我将上传一张航拍图，请你基于该图像自动推理、自动分区、自动规划无人机任务，并 **仅以 JSON 格式** 输出规划结果。

====================================
【你的任务目标】
====================================
请你根据输入航拍图，执行以下 4 步任务。
**核心原则（CRITICAL）**：
1. **全域覆盖（100% Coverage）**：所有 Zone 拼接后必须覆盖整个图像，不能有任何缝隙或未分配的区域。
2. **无缝拼接（Seamless Stitching）**：相邻 Zone 的边界坐标必须完全吻合（共享边界点）。
3. **自然边界（Organic Boundary）**：严禁使用死板的矩形！必须沿着森林边缘、道路、河流等自然地物特征进行划分。

**【核心纠错指令】**
之前的规划中，错误地将“水面”和“陆地”合并了。请注意：
1. **水面（蓝色）** 是完全独立的区域，严禁与陆地合并。
2. **陆地（红/褐色）** 是完全独立的区域，严禁与森林或水面合并。
3. **森林（绿色）** 是完全独立的区域。
------------------------------------
① 场景理解（必须基于图像）
------------------------------------
- 自动分析图像中的主要地貌区块：森林、水体、农田、建筑、道路、坡体等
- 自动识别连通的同质区域（例如整片林地必须合并为一个 Zone）。
- 自动寻找天然边界：林缘线、水岸线、道路、沟谷、崖线。
- **严禁**随意切割连片森林或村落。

------------------------------------
② 区域划分
------------------------------------
你需要划分 3～5 个任务区域（Zone 1 ~ Zone N）：

- 每个 Zone 必须是一个闭合多边形（Polygon）。
- 多边形坐标必须为 0~1 的归一化坐标（例如 [[0.12, 0.35], [0.40, 0.22], ...]）。
- **坐标精度**：请使用至少 2 位小数，确保边界精细贴合。
- 多边形之间 **严禁重叠**，也 **严禁留白**。
...
JSON 列表格式如下，为了保证质量，请在每个 Zone 中包含详细的边界描述：

[
  {
    "id": "zone_0",
    "name": "区域名称 (使用中文，如：西北密林区)",
    "type": "区域类型 (如：森林)",
    "coordinates": [[x1, y1], [x2, y2], ...], 
    "reason": "分配原因 (中文)",
    "boundary_description": "描述该区域的自然边界，例如：'沿西侧河流延伸至北侧林缘' (强制模型思考边界合理性)",
    "coverage_check": "说明该区域覆盖了图像的哪个部分，确保无遗漏"
  },
  ...
]
"""
